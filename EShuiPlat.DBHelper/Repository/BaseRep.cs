...
{
    public abstract class BaseRep<T> : IReposit<T> where T : class
    {
        #region 数据上下文
        ...
        public BaseRep(IDbContext Context)
        {
            _Context = Context;
        }
        ...
        /// 根据SQL验证实体对象是否存在
        /// </summary>
        public abstract   bool IsExist(string sql, params DbParameter[] para);
        /// <summary>
        /// 根据SQL验证实体对象是否存在（异步方式）
        /// </summary>
        public abstract   Task<bool> IsExistAsync(string sql, params DbParameter[] para);
